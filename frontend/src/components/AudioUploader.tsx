...
import { UploadCloud, FileAudio, CheckCircle2, AlertCircle, Loader2, X, Sparkles } from "lucide-react";
...
export default function AudioUploader({ onUploadSuccess }: AudioUploaderProps) {
  ...
  const handleUpload = async () => {
    ...
    setUploading(true);
    setError(null);
    setStatusText("Invio file in corso al server...");

    const formData = new FormData();
    formData.append("file", file);

    try {
      // Aggiorna lo stato visivo dopo 2.5 secondi per dare feedback di trascrittore Whisper ed LLM
      const timer = setTimeout(() => {
        setStatusText("Trascrizione Whisper AI & Polishing editoriale in corso...");
      }, 2500);
      ...
      setError(err.message || "Impossibile caricare l'intervista. Verificare che il backend sia attivo su http://localhost:8000.");
    } finally {
      setUploading(false);
      setStatusText("");
    }
